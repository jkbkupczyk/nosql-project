...
def seed_data(io: TextIO, collection_name: str, number_of_events: int, generator):
    io.write(f"// {collection_name}\n")
    io.write(f"db.{collection_name}.insertMany(")
    io.write(json.dumps(get_events(number_of_events, generator), indent=2))
    io.write(", { ordered: false });\n\n")


if __name__ == '__main__':
    with open('seed.js', 'w', encoding='utf-8') as f:
        seed_data(f, "interbank_events", 100, generate_interbank_event)
        seed_data(f, "swift_events", 100, generate_swift_event)
